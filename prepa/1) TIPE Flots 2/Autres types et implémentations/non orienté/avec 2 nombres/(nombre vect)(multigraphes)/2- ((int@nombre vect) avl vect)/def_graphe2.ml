...
include "biblio2/TIPE/programmes/autres types et implémentations/définitions 2 (avl)/def_avl";;
...
let enleve_et_montre G i j = let l = ref [] in
    let rec aux () = let a, b = enleve_avl G.(i) (j, [||])
      in begin G.(i) <- a; l := b :: !l end in
